...
#### 2.3.1 サンプルデータセット
```2章-教師あり学習/2-3-1サンプルデータセット.ipynb```に記載

#### 2.3.2 k-最近傍法
```2章-教師あり学習/2-3-2k-最近傍法.ipynb```に記載

#### 2.3.3 線形モデル
```2章-教師あり学習/2-3-3線形モデル.ipynb```に記載

#### 2.3.4 ナイーブベイズクラス分類器
```2章-教師あり学習/2-3-3線形モデル.ipynb```に記載

#### 2.3.5 決定木
```2章-教師あり学習/2-3-5決定木.ipynb```に記載

#### 2.3.6 決定木のアンサンブル法
```2章-教師あり学習/2-3-5決定木.ipynb```に記載

#### 2.3.7 カーネル法を用いたサポートベクタマシン
```2章-教師あり学習/2-3-7カーネル法を用いたサポートベクタマシン.ipynb```に記載

#### 2.3.8 ニューラルネットワーク(ディープラーニング)
```2章-教師あり学習/2-3-8ニューラルネットワーク(ディープラーニング).ipynb```に記載

## 2.4 クラス分類器の不確実性推定
```2章-教師あり学習/2-4クラス分類器の不確実性推定.ipynb```に記載

## 2.5 まとめと展望
- 多くのアルゴリズムでは、性能を得るためには正しいパラメータを選択することが重要
- アルゴリズムによっては、入力データの表現(特に特徴量のスケール)に敏感である。

「まとめ」

モデルがおいている仮定やパラメータの意味を理解せずに、

適当なデータセットに適当なアルゴリズムを適用しただけでは正確なモデルを得ることはできない。
...
**ランダムフォレスト**
- ほとんどの場合、単一の決定木よりも高速で頑健で強力
- データのスケールを考慮する必要がない
- 高次元の疎なデータには適さない

**勾配ブースティング決定木**
- 多くの場合ランダムフォレストよりも少し精度が高い
- ランダムフォレストよりも訓練に時間がかかるが、予測はこちらの方が速く、メモリ使用量も小さい
- ランダムフォレストよりもパラメータに敏感

**サポートベクタマシン**
- 同じような意味をもつ特徴量からなる中規模なデータセットに対しては強力
- データのスケールを調整する必要がある
- パラメータに敏感
...
「モデルの取り扱い」
- 新しいデータセットを扱う場合は、線形モデルやナイーブベイズや最近傍法などの簡単なモデルでどのくらい精度が得られるか試すべき。
- データをより深く理解できたら、ランダムフォレストや勾配ブースティング、SVM、ニューラルネットワークなどのより複雑なモデルに移行することを考えるとよい。
- データをどの推定量で試すかに関する問題へのアプローチ方法に関する大まかなガイド[Choosing the right estimator](https://scikit-learn.org/stable/tutorial/machine_learning_map/)

![Choosing the right estimator](Choosing-the-right-estimator.png)